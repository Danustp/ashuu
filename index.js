...
async function startNaze() {
    ...
    naze.sendMessage(callerId, { text: `*Sistem otomatis block!*\n*Jangan menelpon bot*!\n*Silahkan Memohon Ke Owner Untuk Dibuka !*`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       naze.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message by Zenz`, wm_nazedev, [])
       } else if(pea[0].announce == false) {
       naze.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message by Zenz`, wm_nazedev, [])
       } else if (pea[0].restrict == true) {
       naze.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message by Zenz`, wm_nazedev, [])
       } else if (pea[0].restrict == false) {
       naze.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message by Zenz`, wm_nazedev, [])
       } else {
       naze.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup Subject telah diganti menjadi *${pea[0].subject}*`, `Group Settings Change Message by Zenz`, wm_nazedev, [])
     }
    })
        ...
        try {
         ...
         const reSize = async(buffer, ukur1, ukur2) => {
             ...
             resolve(ab)
                ...
                let butwel = [{ buttonId: 'bilek5', buttonText: { displayText: '\nYey akhirnya pacar gua masuk grup😘' }, type: 1 }]
                let butleav = [{ buttonId: 'bilek5', buttonText: { displayText: '\nLah kok pacar gua keluar grup☹️' }, type: 1 }]
                let butselamat = [{ buttonId: 'bilek6', buttonText: { displayText: '\nCie yg dijadikan admin🤭' }, type: 1 }]
                let butsebar = [{ buttonId: 'bilek6', buttonText: { displayText: '\nYahaha kasian udh gak jadi admin lagi😂' }, type: 1 }]
                let nyoutube = (' © Zenz\n https://instagram.com/yanz_5008')
                let teks1 = ` Halo Kak👋🏻 @${num.split('@')[0]}\n Selamat Datang Di Grup ${metadata.subject}\n\n Jangan Lupa Intro Yahh !\n`
                let teks2 = ` Selamat Tinggal Kak @${num.split('@')[0]}\n Semoga Tenang Di Alam Sana😇\n`
                let teks3 = `𝙋 𝙍 𝙊 𝙈 𝙊 𝙏 𝙀 - 𝘿 𝙀 𝙏 𝙀 𝘾 𝙏 𝙀 𝘿 ✔️\n\n• Username: @${num.split('@')[0]}\n• Group: ${metadata.subject}`
                let teks4 = `𝘿 𝙀 𝙈 𝙊 𝙏 𝙀 - 𝘿 𝙀 𝙏 𝙀 𝘾 𝙏 𝙀 𝘿 ✔️\n\n• Username: @${num.split('@')[0]}\n• Group: ${metadata.subject}`
                if (anu.action == 'add') {
                    naze.sendMessage(anu.id, { caption: teks1, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butwel, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'remove') {
                    naze.sendMessage(anu.id, { caption: teks2, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butleav, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'promote') {
                    naze.sendMessage(anu.id, { caption: teks3, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butselamat, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'demote') {
                    naze.sendMessage(anu.id, { caption: teks4, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butsebar, footer: nyoutube, mentions: [num] })
              }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await naze.getName(i + '@s.whatsapp.net')}\nFN:${await naze.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:Ya Allah, izinkan aku jatuh cinta pada si dia yang tepat, di waktu yang tepat. Karena hatiku cuma satu, hanya kepada dia yang halal buatku\nitem2.X-ABLabel:Pesan\nitem3.URL:https://instagram.com/yanz_5008\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Kalimantan Barat - Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
